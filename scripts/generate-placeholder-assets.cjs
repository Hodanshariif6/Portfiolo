...
function createSvgPlaceholder(width, height, title, subtitle, bgColor, textCol, iconType = 'code') {
  ...
  <defs>
    <linearGradient id="grad_${Math.random().toString(36).substr(2, 9)}" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="${bgColor[0]}"/>
      <stop offset="100%" stop-color="${bgColor[1]}"/>
    </linearGradient>
  </defs>
  <rect width="100%" height="100%" fill="${bgColor[0]}"/>
  ...
  <g transform="translate(${width / 2 - 60}, ${height / 2 - 70})">
    ${selectedIcon}
  </g>
  <text x="50%" y="${height / 2 + 30}" font-family="sans-serif" font-weight="bold" font-size="24" fill="${textCol}" text-anchor="middle">${title}</text>
  <text x="50%" y="${height / 2 + 60}" font-family="sans-serif" font-size="14" fill="${textCol}" opacity="0.8" text-anchor="middle">${subtitle}</text>
  <text x="50%" y="${height - 25}" font-family="sans-serif" font-weight="bold" font-size="12" fill="${textCol}" opacity="0.5" text-anchor="middle">HS PORTFOLIO • HODAN SHARIIF</text>
</svg>`;
}
